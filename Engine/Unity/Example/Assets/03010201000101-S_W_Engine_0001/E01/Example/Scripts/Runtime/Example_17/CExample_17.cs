...
/*
 * 내비게이션이란?
 * - Unity 가 지원하는 AI 관련 기능 중 하나로서 경로를 탐색하는 시스템을 의미한다. (즉, 
 * 내비게이션을 활용하면 특정 대상을 원하는 위치로 이동시키는 결과를 손쉽게 구현하는 것이 
 * 가능하다.)
 * 
 * Unity 는 특정 경로를 탐색하기 위해서 내부적으로 A* 알고리즘을 활용하기 때문에 빠른 시간에
 * 특정 경로를 탐색하는 것이 가능하다는 것을 알 수 있다.
 * 
 * Unity 에서 특정 게임 객체가 내비게이션 기능을 활용해서 경로를 탐색하고 싶다면 NavMeshAgent
