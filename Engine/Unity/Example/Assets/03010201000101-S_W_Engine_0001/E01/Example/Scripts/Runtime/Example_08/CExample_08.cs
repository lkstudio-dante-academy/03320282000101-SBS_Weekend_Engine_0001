...
/*
 * Unity UI 시스템이 정상적으로 구동되기 위해서는 반드시 씬 상에 캔버스와 이벤트 시스템 컴포넌트를 지니고 있는 게임 객체 
 * 존재해야한다. (즉, 해당 컴포넌트 중 하나라도 존재하지 않을 경우 Unity UI 는 정상적으로 동작하지 않는다는 것을 알 수 있다.)
 * 
 * 캔버스는 Unity 가 지원하는 여러 UI 컴포넌트가 실질적으로 배치되는 도화지 역할이다. 따라서, Unity 의 모든 UI 는 캔버스 상에 
 * 배치되고 그려지며 해당 캔버스는 필요에 따라 2 개 이상 배치하는 것이 가능하다.
 * 
 * 즉, 모든 UI 는 특정 캔버스 하위에 존재해야되는 제안이 있다는 것을 알 수 있다.
 * 
 * 이벤트 시스템은 사용자와 Unity UI 가 상호작용 할 수 있도록 처리해주는 역할을 수행한다. 따라서, 해당 컴포넌트가 존재하지 않는다면
 * 사용자의 입력에 Unity UI 가 반응하지 않기 때문에 버튼 등의 사용자 입력이 필요한 UI 컴포넌트가 정상적으로 구동되지 않는다는 것을
 * 알 수 있다.
 * 
 ...
 * 
 * 또한, Unity 는 사용자 입력에 해당하는 UI 컴포넌트를 식별하기 위해서 내부적으로 Raycast 방식을 사용하기 때문에 사용자 입력을 처리
 * 하기 위해서는 이벤트 시스템과 더블어 Raycaster 계열 컴포넌트가 반드시 필요하다.
 * 
 * Unity Raycaster 컴포넌트 종류
 ...
 * 2D/3D Raycaster 를 활용하면 Unity UI 시스템에 의해서 해당 객체를 구별하는 것이 가능하다. (즉, Unity UI 시스템과 상호작용을
 * 할 수 있다는 것을 의미한다.)
 * 
 * Unity UI 좌표 시스템은 크게 앵커와 피봇을 통해서 각 UI 의 위치를 계산한다.
 * 앵커는 부모 UI 영역 중 어떤 곳을 기준으로 위치를 계산 할지 결정하는데 사용되며 피봇은 특정 UI 자신의 기준점을 의미한다.
 * 
 * 따라서, Unity UI 는 앵커와 피봇을 이용해서 캔버스 상에 배치 될 위치가 결정되며 해당 기능들을 활용하면 여러 해상도를 손쉽게
 * 처리하는 것이 것이 가능하다. (즉, 해상도 크기에 상관 없이 특정 UI 를 항상 동일한 위치에 배치하는 등의 기능을 구현하는 것이
 * 가능하다.)
 * 
 * Rect Transform 이란?
 * - 일반적으로 Unity UI 를 캔버스 상에 배치하기 위해서 사용되는 Transform 계열 컴포넌트를 의미하며 해당 컴포넌트를 활용하면
 * 앵커와 피봇 같은 Unity UI 시스템에 맞는 정보를 설정하는 것이 가능하다.
 * 
...
public class CE01Example_08 : CSceneManager
{
	...
	public override void Awake()
	{
		...
		/*
		 * Unity GUI 컴포넌트의 이벤트를 처리하기 위한 메서드는 가능하면 스크립트에서 연결해주는 것을 추천한다.
		 * (즉, Unity 씬 상에 해당 작업을 수행 할 경우 씬 파일 구조가 변경이 발생하기 때문에 만약 2 명 이상의
