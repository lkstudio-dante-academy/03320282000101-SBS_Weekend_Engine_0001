...
/*
 * 스크립트란?
 * - 사용자에 의해서 정의되는 컴포넌트를 의미한다. (즉, Unity 에서 미리 만들어서 제공하는 컴포넌트가 아니라 사용자가 제작하는
...
public class CE01Example_03 : CSceneManager
{
	...
	public override void Update()
	{
		...
		if(Input.GetKeyDown(KeyCode.Space))
		{
			 ...
			 * Unity 는 게임 객체를 생성하는 방법으로 크게 2 가지 방식을 제공한다.
			 * 
			 * Unity 에서 게임 객체 생성 방법
			 * - new 키워드
			 * - Instantiate 메서드
			 * 
			 ...
			 * 
			 * 따라서, 일반적으로 Unity 에서 게임 객체를 생성 할 때는 Instantiate 메서드를 사용하는 방식이 널리 활용
			 * 되고 있다.
			 */
		...
		/*
		 * Unity 는 특정 물체를 회전하기 위한 오일러 회전 방식과 사원수 회전 방식을 제공한다.
		 ...
		 * 회전을 계산하기 좀 더 수월하다.
		 * 
		 * 또한, Unity 는 오일러 회전으로 입력한 값이라고 하더라도 내부적으로는 사원수 회전으로 처리한다. (즉, 사원수 회전이 
		 * 오일러 회전에 비해서 회전에 대한 연산을 빠르게 처리 할 수 있기 때문에 Unity 를 비롯한 여러 엔진들이 내부적으로
	...
	/*
	 * Unity 씬 뷰애 특정 정보를 그리기 위한 메서드이다. (즉, 해당 메서드에서 씬 뷰에 프로젝트 개발을 위한 여러 부가 정보를
	...
	private void HandleOnTriggerEnter(CTriggerDispatcher a_oSender, Collider a_oCollider)
	{
		...
		/*
		 * Unity 는 게임 객체를 제거하기 위해서 Destroy 메서드와 DestroyImmediate 메서드를 제공한다.
		 * 
		 ...
		 * 경우가 다반사이다. (즉, Unity 에서 특정 객체를 대상으로 여러 작업을 수행하기 때문에 해당 작업이 수행
