...
public abstract class CSceneManager : CComponent
{
	...
	/*
	 * Active 씬 이란?
	 * - 기본적으로 Unity 는 여러 씬을 중첩해서 특정 게임 공간을 표현하는 것이 가능하다.
	 * 
	 * 이때, 가장 기본이 되는 씬을 Active 씬이라고 하며 Unity 의 특정 메서드는 Active 씬을 대상으로 동작하기 때문에 특정 씬을 대상으로
	 * 메서드를 호출하기 위해서는 Active 씬을 대상으로 하는 메서드를 사용하면 안된다. (Ex. GameObject.Find 메서드 등등...)
	 * 
	 * 또한, Unity 의 모든 게임 객체는 내부적으로 scene 프로퍼티를 지니고 있으며 해당 프로퍼티를 활용하면 특정 객체가 속해 있는 씬의 정보를
	 * 가져오는 것이 가능하다.
	 */
	#region 프로퍼티
